...
# 输出：data/v3_match_tool_coverage.parquet     —— 每专业 × 每工具的覆盖矩阵
#       data/v3_match_summary.parquet           —— 每专业的综合匹配指标
#       data/v3_gap_high_demand_low_supply.csv  —— 高需求-低覆盖工具清单
#       data/v3_gap_low_demand_high_supply.csv  —— 低需求-高供给工具清单
#       Figures/journal/v3/fig_v3_*.png/.pdf    —— 可视化
# 依赖：scripts/_common.py
#       data/v3_supply_majors.parquet（来自 15）
...
FIG_DIR = pathlib.Path("Figures/journal/v3")
...
def load_supply_side():
    df = pd.read_parquet(DATA_DIR / "v3_supply_majors.parquet")
    return df
...
def fig_match_index_bar(summary: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "fig_v3_01_match_index_bar.png", dpi=300, bbox_inches="tight")
    plt.savefig(FIG_DIR / "fig_v3_01_match_index_bar.pdf", bbox_inches="tight")
    plt.close()
    print("  ✓ fig_v3_01_match_index_bar")
...
def fig_tool_coverage_heatmap(cov: pd.DataFrame, top_tools: pd.Series, top_n_show: int = 25):
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "fig_v3_02_tool_coverage_heatmap.png", dpi=300, bbox_inches="tight")
    plt.savefig(FIG_DIR / "fig_v3_02_tool_coverage_heatmap.pdf", bbox_inches="tight")
    plt.close()
    print("  ✓ fig_v3_02_tool_coverage_heatmap")
...
def fig_demand_supply_quadrant(top_tools: pd.Series, supply_counts: dict, n_majors: int):
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "fig_v3_03_demand_supply_quadrant.png", dpi=300, bbox_inches="tight")
    plt.savefig(FIG_DIR / "fig_v3_03_demand_supply_quadrant.pdf", bbox_inches="tight")
    plt.close()
    print("  ✓ fig_v3_03_demand_supply_quadrant")
...
def main():
    ...
    cov.to_parquet(DATA_DIR / "v3_match_tool_coverage.parquet", index=False)
    print(f"  → {DATA_DIR / 'v3_match_tool_coverage.parquet'}（{len(cov):,} 行）")

    print("\n[4/5] 计算每专业综合匹配指标 …")
    summary = build_summary(supply, cov, top_tools, top_soft, df_digi)
    summary.to_parquet(DATA_DIR / "v3_match_summary.parquet", index=False)
    summary.to_csv(DATA_DIR / "v3_match_summary.csv", index=False, encoding="utf-8-sig")
    print(f"  → {DATA_DIR / 'v3_match_summary.parquet'}")
    print("\n  匹配指数 Top10：")
    ...
    high_low.to_csv(DATA_DIR / "v3_gap_high_demand_low_supply.csv", index=False, encoding="utf-8-sig")
    low_high.to_csv(DATA_DIR / "v3_gap_low_demand_high_supply.csv", index=False, encoding="utf-8-sig")
