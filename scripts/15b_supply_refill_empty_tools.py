...
#       最后把所有窗口的工具取并集。
# 输出：覆盖更新 data/v3_supply_majors.parquet
...
def main():
    ...
    # 同时加载 raw 以拿到完整聚合文本
    raw = pd.read_parquet("data/v3_supply_majors_raw.parquet")
    ...
    results = batch_extract(
        ...
        id_column="scan_id",
        text_column="text",
        task_name="v3_supply_tools_refill",
    ...
    for mid, tools_set in by_major.items():
        ...
        if len(idx) == 0:
            continue
        df_full.at[idx[0], "tools"] = np.array(tool_list, dtype=object)
        df_full.at[idx[0], "tools_normalized"] = np.array(tool_list, dtype=object)
